...
$query = "select CONCAT(fname , ' ', mname , ' ', lname) as Name , college_id as ID, dept from Student_details";
...
if(@mysqli_num_rows($query_run)>0 and $i == 1)
{
    ...
    <?php echo '<p style = "text-align:center;color:#fff;">Student Details'?>
    <thead>
        <tr>
            <th>ID</th>
            <th>Name</th>
    ...
    while($row = mysqli_fetch_assoc($query_run)){
        ...
        if($row['dept'] == $dept and substr($row['ID'], 0, 4) == (string)$year)
        {
	        ...
	        echo "<tr>
	                ...
	                <td>".$row['Name']."</td>
	                <td>
	                <form action = ".'"tempst_prof.php" method = "POST">
	                <input type="submit" style="color:black;height:100%;width:100%;background-color: #ffffff;" value="View" name = "'.$row['ID'].'">
	                </td>
