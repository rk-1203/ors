...
if(isset($_POST))
{
  foreach($_POST as $name => $key)
  {
    ...
    $query = "delete from Applied where student_id='".$_SESSION['user_id']."' and job_id=".$name;
...
$drop = "drop view viewApps";
$query_run = @mysqli_query($connect, $drop);

$user_id = $_SESSION['user_id'];

if(!empty($vbprof) && !empty($vbtype))
    $query = "create view viewApps as select Applied.job_id as Job_id, company as Company, profile as Profile, type as Type from Jobs, Applied where student_id='$user_id' and Applied.job_id=Jobs.job_id and profile='$vbprof' and type='$vbtype'";
else if(!empty($vbtype))
    $query = "create view viewApps as select Applied.job_id as Job_id, company as Company, profile as Profile, type as Type from Jobs, Applied where student_id='$user_id' and Applied.job_id=Jobs.job_id and type='$vbtype'";
else if(!empty($vbprof))
    $query = "create view viewApps as select Applied.job_id as Job_id, company as Company, profile as Profile, type as Type from Jobs, Applied where student_id='$user_id' and Applied.job_id=Jobs.job_id and profile='$vbprof'";
else
    $query = "create view viewApps as select Applied.job_id as Job_id, company as Company, profile as Profile, type as Type from Jobs, Applied where student_id='$user_id' and Applied.job_id=Jobs.job_id";

$query_run = @mysqli_query($connect, $query);


$query = "select * from viewApps";
$query_run = @mysqli_query($connect, $query);
...
if(@mysqli_num_rows($query_run)>0)
{
    ...
    <?php echo '<p style = "text-align:center;color:#fff;">Applied Vacancies'?>
    <thead>
        <tr>
            <th>Company</th>
            <th>Profile</th>
